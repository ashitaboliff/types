...
import { createPublicBooking } from "./helper.js";

//#region src/modules/booking/examples/booking.ts
const buildBookingQueryExample = () => {
...
const buildBookingResponseExample = () => {
	const clock = createExampleClock();
	const today = clock.today;
	const tomorrow = clock.daysFromToday(1);
	const dayAfterTomorrow = clock.daysFromToday(2);
	const booking = createPublicBooking(clock, {
		bookingDate: clock.isoDate(today),
		bookingTime: 1
	});
	const tomorrowBooking = createPublicBooking(clock, {
		id: "33333333-3333-4333-8333-333333333333",
		bookingDate: clock.isoDate(tomorrow),
		bookingTime: 0,
		name: "リハーサル"
	});
	const dayAfterBooking = createPublicBooking(clock, {
...
const buildPublicBookingExample = () => createPublicBooking(createExampleClock());
const buildBookingLogsExample = () => {
	const clock = createExampleClock();
	return [createPublicBooking(clock), createPublicBooking(clock, {
...
const buildBookingByUserResponseExample = () => {
...
const buildBookingCreateExample = () => {
	const clock = createExampleClock();
	return {
		userId: "user_mock",
		bookingDate: clock.isoDate(clock.daysFromToday(1)),
...
const buildBookingCreateResponseExample = () => ({ id: "66666666-6666-4666-8666-666666666666" });
const buildBookingUpdateExample = () => {
	const clock = createExampleClock();
	return {
...
const buildBookingPasswordExample = () => ({ password: "mock-password" });
const buildBookingAccessTokenResponseExample = () => {
	const clock = createExampleClock();
	return {
