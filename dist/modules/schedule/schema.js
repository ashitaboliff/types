import { buildScheduleCreateRequestExample, buildScheduleCreatedResponseExample, buildScheduleErrorExample, buildScheduleResponseExample, buildScheduleUserExample, buildScheduleUserListExample } from "./examples.js";
import { z } from "@hono/zod-openapi";

//#region src/modules/schedule/schema.ts
const ScheduleIdParamSchema = z.object({ scheduleId: z.string().min(1) });
const ScheduleIdParam = ScheduleIdParamSchema.openapi({
	param: {
		name: "scheduleId",
		in: "path",
...
const ScheduleUserResponseSchema = z.object({
	id: z.string(),
...
const ScheduleUserListSchema = z.array(ScheduleUserResponseSchema).openapi({
	title: "ScheduleUserList",
	example: buildScheduleUserListExample()
});
const ScheduleCreatedResponseSchema = z.object({ id: z.string() }).openapi({
	title: "ScheduleCreatedResponse",
	example: buildScheduleCreatedResponseExample()
});
const ScheduleErrorResponseSchema = z.object({ error: z.string() }).openapi({
	title: "ScheduleError",
